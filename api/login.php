...
<head>
    ...
    <script>
        // Konfigurasi kustom Tailwind CSS (tema warna, font, animasi)
        tailwind.config = {
            theme: {
                extend: {
                    // Warna emerald kustom dari gelap ke terang
                    colors: {
                        emerald: {
    ...
    <style>
        ...
        /* Kelas untuk animasi card masuk */
...
<body class="min-h-screen flex overflow-hidden bg-emerald-50">
    ...
    <!-- BACKGROUND MESH (Efek gradasi latar belakang) -->
    ...
    <!-- FLOATING ORBS (Bola-bola mengambang dekoratif) -->
    ...
    <!-- HERO PANEL (Panel kiri - hanya tampil di desktop) -->
    <div class="hidden md:flex w-5/12 flex-col justify-between p-12 relative overflow-hidden"
        ...
        <div class="relative z-10 flex flex-col h-full justify-between">
            ...
            <!-- STATS 2x2 (Statistik ringkas sistem) -->
            ...
            <!-- LIVE INDICATOR (Indikator sistem aktif) -->
    ...
    <!-- AUTH PANEL (Panel kanan - form login/register) -->
    <div class="flex-1 flex items-center justify-center p-6 relative z-10">
        <div class="card-in w-full max-w-sm sm:max-w-md rounded-3xl p-8 sm:p-10"
            ...
            <!-- FLASH MESSAGES (Pesan error/sukses dari URL parameter) -->
            <?php
            if (isset($_GET['error'])) {
            ...
            <!-- TAB SWITCHER (Tombol pindah Login / Register) -->
            ...
            <div id="panel-register" class="<?= $tab === 'register' ? '' : 'hidden' ?>">
                <form action="prosesregistrasi.php" method="POST" id="regfrm" class="space-y-3">
                    <!-- 2 Kolom: Nama Depan + Nama Belakang -->
                    ...
                    <!-- Pesan error registrasi (jika ada) -->
    ...
    <!-- JAVASCRIPT (Interaksi Tab, Toggle Password, Password Strength) -->
    <script>
        // Fungsi untuk berpindah tab antara Login dan Register
        function switchTab(t) {
            ...
            // Hapus kelas sebelumnya menggunakan regex
            ...
            // Tambahkan kelas sesuai tab yang aktif
        ...
        // Fungsi untuk toggle visibility password (show/hide)
        ...
        // Fungsi untuk mengecek kekuatan password (password strength meter)
        function chkStr(p) {
            var s = 0;
            if (p.length >= 6) s++;
            if (/[A-Z]/.test(p)) s++;
            ...
            var c = ['', '#EF4444', '#F97316', '#EAB308', '#10B981'];  // Warna sesuai level
            var l = ['', 'Sangat lemah', 'Lemah', 'Sedang', 'Kuat'];    // Label level
            var w = ['0%', '25%', '50%', '75%', '100%'];                 // Lebar progress bar
        ...
        // Event listener untuk validasi konfirmasi password
        ...
        // Event listener submit untuk mengecek kecocokan password sebelum dikirim
