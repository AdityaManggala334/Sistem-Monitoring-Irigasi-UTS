<?php

// KONFIGURASI AWAL DAN INISIALISASI
ob_start();  // Memulai output buffer agar header() dapat digunakan kapan saja
error_reporting(E_ALL);  // Menampilkan semua error PHP (untuk debugging)
ini_set('display_errors', '0');  // Tapi tidak ditampilkan ke browser (aman untuk production)

require_once 'koneksi.php';      // Memuat file koneksi database
require_once 'auth_helper.php';  // Memuat file helper autentikasi (session, role, dll)

// Inisialisasi variabel untuk pesan laporan
$pesan_laporan = '';  // Menyimpan teks pesan
$pesan_warna   = '';   // Menyimpan tipe pesan (sukses/error)

// PROSES FORM LAPORAN KENDALA (ketika user submit form)

if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['kirim_laporan'])) {
    // Ambil data dari form dan bersihkan spasi di awal/akhir
    $nama_pelapor  = trim($_POST['nama_pelapor']  ?? '');
    $lokasi        = trim($_POST['lokasi_kendala'] ?? '');
    $jenis_kendala = trim($_POST['jenis_kendala']  ?? '');

    // Validasi: pastikan semua kolom diisi
    if (empty($nama_pelapor) || empty($lokasi) || empty($jenis_kendala)) {
        $pesan_laporan = 'Mohon isi semua kolom sebelum mengirim laporan.';
        $pesan_warna   = 'error';
    } else {
        // Ambil ID user yang sedang login (dari auth_helper.php)
        $id_user = (int)$user_id ?: null;
        
        // Siapkan query INSERT ke database
        $stmt = mysqli_prepare($conn, "INSERT INTO laporan_kendala (id_users, nama_pelapor, lokasi, jenis_kendala) VALUES (?, ?, ?, ?)");
        mysqli_stmt_bind_param($stmt, 'isss', $id_user, $nama_pelapor, $lokasi, $jenis_kendala);
        
        // Eksekusi query
        if (mysqli_stmt_execute($stmt)) {
            $pesan_laporan = 'Laporan berhasil dikirim! Petugas akan segera menangani.';
            $pesan_warna   = 'sukses';
        } else {
            $pesan_laporan = 'Gagal menyimpan laporan. Coba lagi.';
            $pesan_warna   = 'error';
        }
        mysqli_stmt_close($stmt);  // Tutup statement
    }
}
?>

<!DOCTYPE html>
<html lang="id">
<head>
    <!-- META DATA DAN KONFIGURASI HALAMAN -->
    <meta charset="UTF-8">                    <!-- Set karakter UTF-8 untuk mendukung huruf Indonesia -->
    <meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=yes">  <!-- Responsif untuk mobile -->
    <title>SM Irigasi — Beranda</title>

    <!-- CSS Framework Tailwind CDN -->
    <script src="https://cdn.tailwindcss.com"></script>
    
    <!-- Google Font: Plus Jakarta Sans -->
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link href="https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@300;400;500;600;700;800&display=swap" rel="stylesheet">
    
    <style>
    /* CSS CUSTOM (melengkapi Tailwind) */
    html { scroll-behavior: smooth; }           /* Smooth scroll saat klik anchor link */
    body { font-family: 'Plus Jakarta Sans', sans-serif; }  /* Set font utama */
        
    /* Animasi heartbeat untuk indikator live */
    @keyframes livePulse { 0%,100%{opacity:1} 50%{opacity:0.3} }
    .live-dot { animation: livePulse 2s ease-in-out infinite; }
        
    /* Style untuk badge status sensor */
    .status-pill { 
        display: inline-flex; align-items: center; gap: 4px; 
        padding: 3px 10px; border-radius: 20px; font-size: 0.70rem; 
        font-weight: 700; white-space: nowrap; 
    }
    /* Warna badge sesuai status */
    .sp-normal { background:#F0FDF4; color:#15803D; border:1px solid #BBF7D0; }
    .sp-rendah { background:#FFF7ED; color:#C2410C; border:1px solid #FED7AA; }
    .sp-tinggi { background:#EFF6FF; color:#1D4ED8; border:1px solid #BFDBFE; }
    .sp-kritis { background:#FEF2F2; color:#B91C1C; border:1px solid #FCA5A5; }
        
    /* Dropdown profil saat hover */
    .profil-wrap:hover .profil-dropdown { display: block; }
    .profil-dropdown { 
        display: none; position: absolute; right: 0; top: 100%; margin-top: 8px; 
        background: white; border-radius: 16px; min-width: 200px; 
        box-shadow: 0 8px 32px rgba(0,0,0,0.14); z-index: 50; overflow: hidden; 
    }
        
    /* STYLE TABEL SENSOR */
    #tabel-sensor th, #tabel-sensor td { padding: 10px 12px; white-space: nowrap; }
    #tabel-sensor th { 
        font-size: 0.70rem; text-transform: uppercase; letter-spacing: 0.05em; 
        color: #94a3b8; background: #f8fafc; border-bottom: 1px solid rgba(6,78,59,0.06); 
        font-weight: 700; 
    }
    #tabel-sensor td { 
        font-size: 0.82rem; border-bottom: 1px solid rgba(6,78,59,0.04); 
        color: #475569; 
    }
    #tabel-sensor tbody tr:hover { background: rgba(16,185,129,0.03); }
    #tabel-sensor td:nth-child(1) { color: #94a3b8; width: 40px; text-align: center; }
    #tabel-sensor td:nth-child(2) { font-weight: 700; color: #059669; }
        
    /* RESPONSIVE MOBILE (Media Queries) */
    /* Tablet (max 768px) */
    @media (max-width: 768px) {
        .nav-links .nav-link span:not(.inline-svg) { display: none; }  /* Sembunyikan teks menu di mobile */
        .nav-links .nav-link { padding: 6px 8px !important; }
        .relative.rounded-3xl > div.p-10 { padding: 20px !important; }
        .text-3xl { font-size: 1.5rem !important; }
        .grid-cols-2.lg\:grid-cols-4 { grid-template-columns: repeat(2, minmax(0, 1fr)) !important; gap: 12px !important; }
        .grid-cols-3 { grid-template-columns: repeat(3, 1fr) !important; }
        #tabel-sensor th, #tabel-sensor td { padding: 8px 6px !important; font-size: 0.7rem !important; }
        .sm\:grid-cols-2 { grid-template-columns: 1fr !important; }
        .flex.gap-3.flex-wrap { flex-direction: column; gap: 10px !important; }
        .flex.gap-3.flex-wrap a { width: 100%; justify-content: center; }
    }
        
    /* Handphone (max 480px) */
    @media (max-width: 480px) {
        .nav-links .nav-link { padding: 4px 6px !important; }
        .relative.rounded-3xl .flex.items-center { flex-direction: column !important; text-align: center; }
        .max-w-lg { text-align: center; }
        #tabel-sensor th { font-size: 0.6rem !important; }
    }
        
    /* Smooth scrolling untuk tabel di mobile */
    .overflow-x-auto { -webkit-overflow-scrolling: touch; }
    </style>
</head>
<body class="bg-slate-50 text-slate-800">

<!-- NAVBAR (Navigasi Utama) -->
<nav class="sticky top-0 z-50 border-b" style="background:rgba(6,78,59,0.97);backdrop-filter:blur(16px);border-color:rgba(255,255,255,0.07);">
  <div class="max-w-6xl mx-auto px-4 sm:px-6 h-14 sm:h-16 flex items-center justify-between gap-2 sm:gap-4">

    <!-- Logo / Brand -->
    <a href="index.php" class="flex items-center gap-2 sm:gap-2.5 no-underline flex-shrink-0">
      <div class="w-8 h-8 sm:w-9 sm:h-9 rounded-xl flex items-center justify-center" style="background:rgba(16,185,129,0.18);border:1px solid rgba(52,211,153,0.25);">
        <svg width="18" height="18" viewBox="0 0 44 44" fill="none">
          <path d="M22 7C22 7 13 18 13 24C13 29.52 17.03 34 22 34C26.97 34 31 29.52 31 24C31 18 22 7 22 7Z" fill="#10B981"/>
          <line x1="18" y1="24" x2="26" y2="24" stroke="white" stroke-width="1.8" stroke-linecap="round"/>
          <circle cx="18" cy="24" r="1.4" fill="white"/>
          <circle cx="26" cy="24" r="1.4" fill="white"/>
          <line x1="22" y1="20" x2="22" y2="28" stroke="white" stroke-width="1.8" stroke-linecap="round"/>
        </svg>
      </div>
      <span class="text-sm sm:text-base font-extrabold text-white tracking-tight">SM Irigasi</span>
    </a>

    <!-- Menu Navigasi -->
    <div class="flex items-center gap-0 sm:gap-1">
      <!-- Link ke bagian Tentang (hanya tampil di desktop) -->
      <a href="#tentang" class="nav-link hidden sm:flex items-center gap-1 px-2 sm:px-3 py-1.5 rounded-lg text-xs sm:text-sm font-medium transition-all hover:bg-white/10" style="color:rgba(255,255,255,0.65);">
        <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="22 12 18 12 15 21 9 3 6 12 2 12"/></svg>
        <span>Tentang</span>
      </a>
      
      <!-- Link ke Monitoring -->
      <a href="#monitoring" class="nav-link flex items-center gap-1 px-2 sm:px-3 py-1.5 rounded-lg text-xs sm:text-sm font-medium transition-all hover:bg-white/10" style="color:rgba(255,255,255,0.65);">
        <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="22 12 18 12 15 21 9 3 6 12 2 12"/></svg>
        <span class="hidden sm:inline">Monitor</span>
      </a>
      
      <!-- Link ke Peta -->
      <a href="peta.php" class="nav-link flex items-center gap-1 px-2 sm:px-3 py-1.5 rounded-lg text-xs sm:text-sm font-medium transition-all hover:bg-white/10" style="color:rgba(255,255,255,0.65);">
        <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polygon points="3 6 9 3 15 6 21 3 21 18 15 21 9 18 3 21"/></svg>
        <span class="hidden sm:inline">Peta</span>
      </a>
      
      <!-- Link ke BPS -->
      <a href="bps.php" class="nav-link flex items-center gap-1 px-2 sm:px-3 py-1.5 rounded-lg text-xs sm:text-sm font-medium transition-all hover:bg-white/10" style="color:rgba(255,255,255,0.65);">
        <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="3" width="18" height="18" rx="2"/><path d="M3 9h18M9 21V9"/></svg>
        <span class="hidden sm:inline">BPS</span>
      </a>
      
      <!-- Link ke Riwayat -->
      <a href="riwayat.php" class="nav-link flex items-center gap-1 px-2 sm:px-3 py-1.5 rounded-lg text-xs sm:text-sm font-medium transition-all hover:bg-white/10" style="color:rgba(255,255,255,0.65);">
        <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>
        <span class="hidden sm:inline">Riwayat</span>
      </a>
      
      <!-- Link ke Dashboard Admin (hanya untuk role administrator) -->
      <?php if ($role === 'administrator'): ?>
      <a href="dashboard.php" class="nav-link hidden md:flex items-center gap-1 px-2 sm:px-3 py-1.5 rounded-lg text-xs sm:text-sm font-medium transition-all hover:bg-white/10" style="color:rgba(255,255,255,0.65);">
        <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><rect x="14" y="14" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/></svg>
        <span>Admin</span>
      </a>
      <?php endif; ?>

      <!-- Profil & Dropdown -->
      <div class="profil-wrap relative ml-1 sm:ml-2">
        <button class="flex items-center gap-1 sm:gap-2 px-2 sm:px-3 py-1.5 rounded-lg text-xs sm:text-sm font-medium hover:bg-white/10 bg-transparent border-none cursor-pointer" style="color:rgba(255,255,255,0.80);">
          <!-- Avatar bulat dengan inisial nama depan -->
          <div class="w-6 h-6 sm:w-7 sm:h-7 rounded-lg flex items-center justify-center font-bold text-xs" style="background:rgba(16,185,129,0.25);color:#34D399;">
            <?= strtoupper(substr($namaDepan ?: 'U', 0, 1)) ?>
          </div>
          <span class="hidden sm:inline"><?= $namaDepan ?></span>
          <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="6 9 12 15 18 9"/></svg>
        </button>
        
        <!-- Dropdown menu profil -->
        <div class="profil-dropdown">
          <div class="px-4 py-3 border-b" style="background:linear-gradient(135deg,#F0FDF6,#E6F9F0);border-color:rgba(6,78,59,0.08);">
            <div class="font-bold text-emerald-900 text-sm"><?= $namaLengkap ?></div>
            <div class="text-xs text-slate-500 mt-0.5 capitalize"><?= str_replace('_', ' ', $role) ?></div>
          </div>
          <a href="logout.php" class="flex items-center gap-2 px-4 py-3 text-sm text-red-600 hover:bg-red-50 transition-colors no-underline">
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4"/><polyline points="16 17 21 12 16 7"/><line x1="21" y1="12" x2="9" y2="12"/></svg>
            Keluar
          </a>
        </div>
      </div>
    </div>
  </div>
</nav>

<!-- KONTEN UTAMA -->
<div class="max-w-6xl mx-auto px-4 sm:px-6 py-4 sm:py-8">

    <!-- HERO SECTION (Bagian Pembuka) -->
    <div class="relative rounded-2xl sm:rounded-3xl overflow-hidden mb-6 sm:mb-8" style="min-height:280px;">
        <!-- Background gambar sawah -->
        <img src="https://i.imgur.com/elrEGQB.jpeg" alt="Irigasi Sawah" class="absolute inset-0 w-full h-full object-cover" style="object-position:center 40%;" onerror="this.style.display='none'">
        
        <!-- Overlay gradasi gelap agar teks terbaca -->
        <div class="absolute inset-0" style="background:linear-gradient(100deg,rgba(2,44,34,0.90) 0%,rgba(6,78,59,0.70) 55%,rgba(6,78,59,0.25) 100%);"></div>
        
        <!-- Pola titik-titik dekoratif -->
        <div class="absolute inset-0 opacity-5" style="background-image:linear-gradient(rgba(255,255,255,1) 1px,transparent 1px),linear-gradient(90deg,rgba(255,255,255,1) 1px,transparent 1px);background-size:48px 48px;"></div>

        <!-- Konten teks dalam hero -->
        <div class="relative z-10 flex flex-col sm:flex-row items-center justify-between gap-4 sm:gap-8 p-6 sm:p-10" style="min-height:280px;">
            <!-- Teks kiri -->
            <div class="max-w-lg text-center sm:text-left">
                <div class="inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-semibold mb-4" style="background:rgba(16,185,129,0.20);border:1px solid rgba(52,211,153,0.30);color:#34D399;">
                    <span class="live-dot inline-block w-1.5 h-1.5 bg-emerald-400 rounded-full"></span>
                    Sistem Aktif · Update 4 Detik
                </div>
                <h1 class="text-2xl sm:text-3xl md:text-4xl font-extrabold text-white leading-tight tracking-tight mb-3">
                    Sistem Monitoring<br>Irigasi Sawah <span class="text-emerald-400">Cerdas</span>
                </h1>
                <p class="text-xs sm:text-sm leading-relaxed mb-6 max-w-md" style="color:rgba(255,255,255,0.60);">
                    Pantau debit air, tinggi muka air, dan kelembapan tanah secara real-time dari sensor yang tersebar di seluruh jaringan irigasi sawah.
                </p>
                <div class="flex flex-col sm:flex-row gap-3 flex-wrap">
                    <!-- Tombol scroll ke tabel monitoring -->
                    <a href="#monitoring" class="inline-flex items-center justify-center gap-2 px-4 sm:px-5 py-2 rounded-xl text-sm font-bold text-white transition-all hover:-translate-y-0.5"
                       style="background:linear-gradient(135deg,#10B981,#059669);box-shadow:0 4px 16px rgba(16,185,129,0.40);">
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="22 12 18 12 15 21 9 3 6 12 2 12"/></svg>
                        Lihat Data Sensor
                    </a>
                    <!-- Tombol ke peta -->
                    <a href="peta.php" class="inline-flex items-center justify-center gap-2 px-4 sm:px-5 py-2 rounded-xl text-sm font-bold transition-all hover:bg-white/20"
                       style="background:rgba(255,255,255,0.10);color:rgba(255,255,255,0.85);border:1px solid rgba(255,255,255,0.18);">
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polygon points="3 6 9 3 15 6 21 3 21 18 15 21 9 18 3 21"/></svg>
                        Peta Sensor
                    </a>
                </div>
            </div>
            
            <!-- Statistik ringkas (4 card) -->
            <div class="grid grid-cols-2 gap-2 sm:gap-3 w-full sm:w-auto mt-4 sm:mt-0">
                <?php foreach ([['8','Sensor'],['240ha','Area'],['4 dtk','Update'],['99%','Uptime']] as [$n,$l]): ?>
                <div class="px-3 sm:px-5 py-2 sm:py-3 rounded-2xl text-center" style="background:rgba(255,255,255,0.07);border:1px solid rgba(255,255,255,0.12);backdrop-filter:blur(8px);">
                    <div class="text-lg sm:text-xl font-extrabold text-emerald-400 leading-none"><?= $n ?></div>
                    <div class="text-[10px] sm:text-xs font-medium mt-1" style="color:rgba(255,255,255,0.45);"><?= $l ?></div>
                </div>
                <?php endforeach; ?>
            </div>
        </div>
    </div>

    <!-- AKSES CEPAT (Quick Access Cards) -->
    <div class="mb-3">
        <h2 class="text-sm font-bold text-slate-700">Akses Cepat</h2>
        <p class="text-xs text-slate-400 mt-0.5 mb-4">Navigasi langsung ke fitur utama sistem</p>
    </div>
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 mb-6 sm:mb-8">
        <?php foreach ([
            ['peta.php',    'Peta Sensor',     'Posisi & status sensor',     'Buka',     '#15803D','#F0FDF4','<polygon points="3 6 9 3 15 6 21 3 21 18 15 21 9 18 3 21"/>'],
            ['bps.php',     'Data BPS',        'Data luas panen & produktivitas', 'Lihat', '#1D4ED8','#EFF6FF','<rect x="3" y="3" width="18" height="18" rx="2"/><path d="M3 9h18M9 21V9"/>'],
            ['riwayat.php', 'Riwayat Data',    'Historis pembacaan sensor',   'Lihat', '#6D28D9','#F5F3FF','<polyline points="22 12 18 12 15 21 9 3 6 12 2 12"/>'],
            ['#lapor',      'Laporan',         'Laporkan masalah irigasi',    'Buat',  '#92400E','#FFFBEB','<path d="M14 2H6a2 2 0 0 0-2 2v16h16V8z"/><polyline points="14 2 14 8 20 8"/>'],
        ] as [$href,$title,$desc,$cta,$color,$bg,$ico]): ?>
        <a href="<?= $href ?>" class="bg-white rounded-2xl p-4 sm:p-5 border flex flex-col gap-3 transition-all hover:-translate-y-1 hover:shadow-lg no-underline relative overflow-hidden"
           style="border-color:rgba(6,78,59,0.08);box-shadow:0 1px 3px rgba(6,78,59,0.05),0 4px 16px rgba(6,78,59,0.05);">
            <div class="absolute top-0 left-0 right-0 h-0.5" style="background:<?= $color ?>;opacity:0.6;"></div>
            <div class="w-8 h-8 sm:w-9 sm:h-9 rounded-xl flex items-center justify-center" style="background:<?= $bg ?>;">
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="<?= $color ?>" stroke-width="2"><?= $ico ?></svg>
            </div>
            <div>
                <div class="font-bold text-slate-800 text-sm"><?= $title ?></div>
                <div class="text-xs text-slate-400 mt-0.5 leading-relaxed hidden sm:block"><?= $desc ?></div>
            </div>
            <div class="text-xs font-bold flex items-center gap-1 mt-auto" style="color:<?= $color ?>;">
                <?= $cta ?> <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="9 18 15 12 9 6"/></svg>
            </div>
        </a>
        <?php endforeach; ?>
    </div>

    <!-- TENTANG SISTEM & INFORMASI SISTEM (2 Kolom) -->
    <div id="tentang" class="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6 mb-6 sm:mb-8">
        
        <!-- Kolom Kiri: Tentang Sistem -->
        <div class="bg-white rounded-2xl p-5 sm:p-6 border" style="border-color:rgba(6,78,59,0.08);box-shadow:0 1px 3px rgba(6,78,59,0.05),0 8px 24px rgba(6,78,59,0.06);">
            <h2 class="text-base font-bold text-emerald-900 mb-3 pb-2 inline-block border-b-2 border-emerald-500">Tentang Sistem</h2>
            <p class="text-sm text-slate-500 leading-relaxed mb-4">Platform berbasis web yang mengumpulkan data dari sensor jaringan irigasi sawah secara real-time untuk mendukung efisiensi pertanian.</p>
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
                <?php foreach ([
                    ['Monitor Debit','Sensor flow meter otomatis'],
                    ['TMA Presisi','Sensor ultrasonik akurasi tinggi'],
                    ['Notifikasi','Peringatan ambang batas aman'],
                    ['Peta Visual','Posisi sensor di lapangan'],
                ] as [$t,$d]): ?>
                <div class="flex gap-2 p-2 rounded-xl" style="background:rgba(16,185,129,0.04);border:1px solid rgba(16,185,129,0.10);">
                    <div class="w-1.5 h-1.5 rounded-full bg-emerald-500 flex-shrink-0 mt-1.5"></div>
                    <div class="text-xs text-slate-500"><strong class="text-slate-700"><?= $t ?></strong><br><?= $d ?></div>
                </div>
                <?php endforeach; ?>
            </div>
        </div>

        <!-- Kolom Kanan: Informasi Sistem (Tabel Spesifikasi) -->
        <div class="bg-white rounded-2xl p-5 sm:p-6 border" style="border-color:rgba(6,78,59,0.08);box-shadow:0 1px 3px rgba(6,78,59,0.05),0 8px 24px rgba(6,78,59,0.06);">
            <h2 class="text-base font-bold text-emerald-900 mb-4 pb-2 inline-block border-b-2 border-emerald-500">Informasi Sistem</h2>
            <div class="overflow-x-auto">
                <table class="w-full text-sm border-collapse min-w-[300px]">
                    <thead>
                        <tr>
                            <th class="py-2 px-2 text-left text-xs font-bold uppercase text-white rounded-l-lg" style="background:#064E3B;">#</th>
                            <th class="py-2 px-2 text-left text-xs font-bold uppercase text-white" style="background:#064E3B;">Keterangan</th>
                            <th class="py-2 px-2 text-left text-xs font-bold uppercase text-white rounded-r-lg" style="background:#064E3B;">Detail</th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php foreach ([
                            ['1','Nama Sistem','SM Irigasi'],
                            ['2','Titik Sensor','8 Titik Aktif'],
                            ['3','Jenis Sensor','Ultrasonik, DHT22, Flow Meter'],
                            ['4','Komunikasi','LoRa / GSM / WiFi'],
                            ['5','Update Interval','Setiap 4 detik'],
                            ['6','Area Sawah','±240 Hektar'],
                        ] as [$no,$k,$v]): ?>
                        <tr class="hover:bg-emerald-50/40 transition-colors">
                            <td class="py-2.5 px-2 text-emerald-600 font-bold border-b" style="border-color:rgba(6,78,59,0.06);"><?= $no ?></td>
                            <td class="py-2.5 px-2 text-slate-500 border-b" style="border-color:rgba(6,78,59,0.06);"><?= $k ?></td>
                            <td class="py-2.5 px-2 text-slate-700 font-medium border-b" style="border-color:rgba(6,78,59,0.06);"><?= $v ?></td>
                        </tr>
                        <?php endforeach; ?>
                    </tbody>
                </table>
            </div>
        </div>
    </div>

    <!-- TABEL MONITORING SENSOR (Data Real-Time) -->
    <div id="monitoring" class="bg-white rounded-2xl border overflow-hidden mb-6 sm:mb-8" style="border-color:rgba(6,78,59,0.08);box-shadow:0 1px 3px rgba(6,78,59,0.05),0 8px 24px rgba(6,78,59,0.06);">

        <!-- Header Tabel -->
        <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 px-4 sm:px-5 py-3 sm:py-4 border-b" style="border-color:rgba(6,78,59,0.06);background:linear-gradient(135deg,#F0FDF4,#F8FAFC);">
            <div>
                <div class="font-bold text-slate-700 flex items-center gap-2 text-sm sm:text-base">
                    <span class="live-dot inline-block w-2 h-2 bg-emerald-500 rounded-full"></span>
                    Data Monitoring Sensor Real-Time
                </div>
                <div class="text-xs text-slate-400 mt-0.5">Update 4 detik · 8 titik sensor aktif</div>
            </div>
            <a href="peta.php" class="flex items-center gap-1 text-xs font-bold text-emerald-600 hover:text-emerald-800 transition-colors no-underline px-3 py-1.5 rounded-lg hover:bg-emerald-50">
                Lihat Peta <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="9 18 15 12 9 6"/></svg>
            </a>
        </div>

        <!-- Tabel (scroll horizontal untuk mobile) -->
        <div class="overflow-x-auto" style="-webkit-overflow-scrolling: touch;">
            <table class="w-full border-collapse" id="tabel-sensor" style="min-width:600px;">
                <thead>
                    <tr>
                        <th style="width:40px;text-align:center;">No</th>
                        <th style="width:80px;">ID</th>
                        <th>Lokasi</th>
                        <th style="width:100px;">Debit (L/dtk)</th>
                        <th style="width:80px;">TMA (cm)</th>
                        <th style="width:80px;">Suhu (°C)</th>
                        <th style="width:80px;">Lembap (%)</th>
                        <th style="width:90px;">Status</th>
                        <th style="width:70px;">Waktu</th>
                    </tr>
                </thead>
                <tbody id="isi-tabel"></tbody>
            </table>
        </div>

        <!-- Ringkasan bawah (rata-rata debit, TMA, jumlah normal) -->
        <div class="grid grid-cols-3 divide-x border-t" style="border-color:rgba(6,78,59,0.06);divide-color:rgba(6,78,59,0.06);">
            <div class="px-3 sm:px-5 py-2 sm:py-3 text-center sm:text-left">
                <div class="text-[10px] sm:text-xs text-slate-400">Rata-rata Debit</div>
                <div class="text-sm sm:text-base font-bold text-slate-700 mt-0.5"><span id="rata-debit">—</span> <span class="text-[10px] sm:text-xs font-normal text-slate-400">L/dtk</span></div>
            </div>
            <div class="px-3 sm:px-5 py-2 sm:py-3 text-center sm:text-left">
                <div class="text-[10px] sm:text-xs text-slate-400">Rata-rata TMA</div>
                <div class="text-sm sm:text-base font-bold text-slate-700 mt-0.5"><span id="rata-tma">—</span> <span class="text-[10px] sm:text-xs font-normal text-slate-400">cm</span></div>
            </div>
            <div class="px-3 sm:px-5 py-2 sm:py-3 text-center sm:text-left">
                <div class="text-[10px] sm:text-xs text-slate-400">Status Normal</div>
                <div class="text-sm sm:text-base font-bold text-emerald-600 mt-0.5"><span id="sensor-aman">—</span></div>
            </div>
        </div>
    </div>

    <!-- FORM LAPORAN KENDALA -->
    <div id="lapor" class="bg-white rounded-2xl border overflow-hidden mb-6 sm:mb-8" style="border-color:rgba(6,78,59,0.08);box-shadow:0 1px 3px rgba(6,78,59,0.05),0 8px 24px rgba(6,78,59,0.06);">

        <!-- Header Form -->
        <div class="px-4 sm:px-6 py-3 sm:py-4 border-b" style="border-color:rgba(6,78,59,0.06);background:linear-gradient(135deg,#F0FDF4,#F8FAFC);">
            <div class="font-bold text-slate-700 flex items-center gap-2 text-sm sm:text-base">
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#059669" stroke-width="2"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/></svg>
                Laporan Kendala Irigasi
            </div>
            <div class="text-xs text-slate-400 mt-0.5">Petani atau petugas dapat melaporkan masalah irigasi melalui formulir berikut</div>
        </div>

        <div class="p-4 sm:p-6">
            <form method="POST">
                <!-- 2 kolom: Nama Pelapor + Lokasi -->
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                    <div class="flex flex-col gap-1.5">
                        <label class="text-xs font-bold uppercase tracking-wider text-slate-500">Nama Pelapor</label>
                        <input type="text" name="nama_pelapor" placeholder="Nama lengkap Anda"
                               value="<?= $namaLengkap ?>" required
                               class="px-3 py-2.5 border border-slate-200 rounded-xl text-sm text-slate-700 outline-none focus:border-emerald-400 focus:ring-2 focus:ring-emerald-100 transition-all placeholder:text-slate-300">
                    </div>
                    <div class="flex flex-col gap-1.5">
                        <label class="text-xs font-bold uppercase tracking-wider text-slate-500">Lokasi Kendala</label>
                        <input type="text" name="lokasi_kendala" placeholder="Contoh: Saluran Ngalor D" required
                               class="px-3 py-2.5 border border-slate-200 rounded-xl text-sm text-slate-700 outline-none focus:border-emerald-400 focus:ring-2 focus:ring-emerald-100 transition-all placeholder:text-slate-300">
                    </div>
                </div>
                
                <!-- Jenis Kendala (select dropdown) -->
                <div class="flex flex-col gap-1.5 mb-5">
                    <label class="text-xs font-bold uppercase tracking-wider text-slate-500">Jenis Kendala</label>
                    <select name="jenis_kendala" required
                            class="px-3 py-2.5 border border-slate-200 rounded-xl text-sm text-slate-700 outline-none focus:border-emerald-400 focus:ring-2 focus:ring-emerald-100 transition-all">
                        <option value="">— Pilih Jenis Kendala —</option>
                        <option>Debit air terlalu kecil</option>
                        <option>Debit air terlalu besar / banjir</option>
                        <option>Sensor tidak terbaca</option>
                        <option>Saluran tersumbat</option>
                        <option>Pintu air rusak</option>
                        <option>Lainnya</option>
                    </select>
                </div>
                
                <!-- Tombol Submit -->
                <button type="submit" name="kirim_laporan"
                  class="inline-flex items-center justify-center gap-2 w-full sm:w-auto px-6 py-2.5 rounded-xl text-sm font-bold text-white transition-all hover:-translate-y-0.5 active:translate-y-0"
                  style="background:linear-gradient(135deg,#065F46,#064E3B);box-shadow:0 4px 16px rgba(6,78,59,0.25);">
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><line x1="22" y1="2" x2="11" y2="13"/><polygon points="22 2 15 22 11 13 2 9 22 2"/></svg>
                  Kirim Laporan
                </button>
            </form>

            <!-- Tampilkan pesan sukses/error setelah submit -->
            <?php if ($pesan_laporan): ?>
            <div class="flex items-center gap-2 mt-4 px-4 py-3 rounded-xl text-sm font-medium"
                 style="<?= $pesan_warna === 'sukses'
                   ? 'background:#F0FDF4;border:1px solid #BBF7D0;color:#166534;'
                   : 'background:#FEF2F2;border:1px solid #FECACA;color:#991B1B;' ?>">
                <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                  <?= $pesan_warna === 'sukses'
                    ? '<polyline points="20 6 9 17 4 12"/>'
                    : '<circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/>' ?>
                </svg>
                <?= htmlspecialchars($pesan_laporan) ?>
            </div>
            <?php endif; ?>
        </div>
    </div>

</div><!-- penutup max-w-6xl -->

<!-- FOOTER (Bagian Bawah Halaman) -->
<footer class="text-center py-4 sm:py-5 text-xs" style="background:#064E3B;color:rgba(255,255,255,0.40);">
  © 2026 Sistem Monitoring Irigasi Sawah — Universitas Sebelas Maret
</footer>

<!-- JAVASCRIPT (Data Sensor & Update Real-Time) -->
<script>

// DATA SENSOR (8 titik sensor dengan nilai default)
var dataSensor = [
  {id:"SNS-01", lokasi:"Saluran Induk Ngidul", debit:12.4, tma:42, suhu:26.8, lembap:68, status:"normal"},
  {id:"SNS-02", lokasi:"Percabangan Blok A",   debit:8.7,  tma:35, suhu:27.1, lembap:72, status:"normal"},
  {id:"SNS-03", lokasi:"Saluran Blok B",       debit:3.2,  tma:18, suhu:28.3, lembap:45, status:"rendah"},
  {id:"SNS-04", lokasi:"Bak Penampungan C1",   debit:18.9, tma:71, suhu:26.2, lembap:80, status:"tinggi"},
  {id:"SNS-05", lokasi:"Saluran Ngalor D",     debit:6.5,  tma:28, suhu:27.8, lembap:63, status:"normal"},
  {id:"SNS-06", lokasi:"Saluran Ngetan E",     debit:1.1,  tma:10, suhu:29.0, lembap:31, status:"kritis"},
  {id:"SNS-07", lokasi:"Saluran Petak 12",     debit:9.3,  tma:38, suhu:26.5, lembap:70, status:"normal"},
  {id:"SNS-08", lokasi:"Embung Ngulon",        debit:7.8,  tma:32, suhu:27.4, lembap:66, status:"normal"}
];

// Mapping warna dot dan label untuk setiap status
var dotColor = {normal:"#10B981", rendah:"#F97316", tinggi:"#3B82F6", kritis:"#EF4444"};
var labelSt  = {normal:"Normal",  rendah:"Rendah",  tinggi:"Tinggi",  kritis:"Kritis!"};
var spClass  = {normal:"sp-normal", rendah:"sp-rendah", tinggi:"sp-tinggi", kritis:"sp-kritis"};

// Fungsi untuk mendapatkan waktu saat ini (format HH:MM:SS)
function waktu() {
  var n = new Date();
  return String(n.getHours()).padStart(2,'0') + ':' +
         String(n.getMinutes()).padStart(2,'0') + ':' +
         String(n.getSeconds()).padStart(2,'0');
}

// Fungsi untuk membuat badge status (pill) dengan warna sesuai status
function pill(s) {
  return '<span class="status-pill ' + spClass[s] + '">' +
         '<svg width="6" height="6" viewBox="0 0 6 6"><circle cx="3" cy="3" r="3" fill="' + dotColor[s] + '"/></svg>' +
         labelSt[s] + '</span>';
}

// Fungsi untuk merender tabel sensor ke halaman
function renderTabel() {
  var html = '';
  dataSensor.forEach(function(s, i) {
    html += '<tr>';
    html += '<td style="text-align:center;color:#94a3b8;">' + (i+1) + '</td>';
    html += '<td style="font-weight:700;color:#059669;">' + s.id + '</td>';
    html += '<td>' + s.lokasi + '</td>';
    html += '<td style="font-variant-numeric:tabular-nums;">' + s.debit.toFixed(1) + '</td>';
    html += '<td style="font-variant-numeric:tabular-nums;">' + s.tma + '</td>';
    html += '<td style="font-variant-numeric:tabular-nums;">' + s.suhu.toFixed(1) + '</td>';
    html += '<td style="font-variant-numeric:tabular-nums;">' + s.lembap + '</td>';
    html += '<td>' + pill(s.status) + '</td>';
    html += '<td style="color:#94a3b8;font-size:0.7rem;">' + waktu() + '</td>';
    html += '</tr>';
  });
  document.getElementById('isi-tabel').innerHTML = html;
  hitungRingkasan();  // Update ringkasan setelah render
}

// Fungsi menghitung ringkasan (rata-rata debit, TMA, dan jumlah sensor normal)
function hitungRingkasan() {
  var td = 0, tt = 0, n = 0, c = dataSensor.length;
  dataSensor.forEach(function(s) { 
    td += s.debit;      // total debit
    tt += s.tma;        // total TMA
    if (s.status === 'normal') n++;  // jumlah sensor normal
  });
  document.getElementById('rata-debit').textContent = (td/c).toFixed(1);
  document.getElementById('rata-tma').textContent   = Math.round(tt/c);
  document.getElementById('sensor-aman').textContent = n + ' dari ' + c + ' titik';
}

// Fungsi untuk mensimulasikan update data sensor (random perubahan nilai)
function perbaruiSensor() {
  dataSensor.forEach(function(s) {
    // Ubah debit secara random (±0.5)
    s.debit  = Math.max(0.5, s.debit + (Math.random() - 0.5));
    // Ubah TMA secara random (±3)
    s.tma    = Math.max(5,   s.tma   + Math.round((Math.random() - 0.5) * 3));
    // Ubah kelembapan secara random (±2)
    s.lembap = Math.min(100, Math.max(10, s.lembap + Math.round((Math.random() - 0.5) * 2)));
    
    // Tentukan status baru berdasarkan nilai TMA
    if      (s.tma < 15) s.status = 'kritis';  // TMA < 15 cm = kritis
    else if (s.tma < 25) s.status = 'rendah';  // TMA 15-24 cm = rendah
    else if (s.tma > 65) s.status = 'tinggi';  // TMA > 65 cm = tinggi
    else                  s.status = 'normal'; // TMA 25-65 cm = normal
  });
  renderTabel();  // Render ulang tabel setelah update
}

// Jalankan render pertama kali
renderTabel();

// Set interval untuk update data setiap 4 detik (4000 ms)
setInterval(perbaruiSensor, 4000);
</script>

</body>
</html>
